...
test('authorized user can delete threads', async ({ assert, client }) => {
  ...
  const response = await client
    .delete(thread.url()).send().loginVia(await thread.user().first()).end()
...
test('authorized user can update title and body of threads', async ({ assert, client }) => {
  ...
  const response = await client.put(thread.url())
    .loginVia(await thread.user().first()).send(attributes).end()
...
test('can not create thread with no body or title', async ({ client }) => {
  const user = await Factory.model('App/Models/User').create()
  let response = await client.post('/threads')
...
test('can not update thread with no body or title', async ({ client }) => {
  ...
  const put = () => client.put(thread.url())
    .header('accept', 'application/json')
    .loginVia(user)
...
test('can access all resources', async ({ client }) => {
  ...
  response.assertStatus(200)
  response.assertJSON({ threads: threads.map(thread => thread.toJSON())
    .sort((a, b) => a.id - b.id) })
})
...
test('moderator can update title and body of threads', async ({ assert, client }) => {
  ...
  const response = await client.put(thread.url()).loginVia(moderator)
    .send(attributes).end()
